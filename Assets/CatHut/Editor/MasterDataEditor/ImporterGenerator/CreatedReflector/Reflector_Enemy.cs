...
namespace CatHut
{
	...
	{
		static void ReflectEnemy(string child, FormatedCsvData fcd)
		{
                ...
                case "EnemyParameter":
                    {
                        ...
                        foreach (var row in fcd.DataPart.DataWithoutColumnTitle)
                        {
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:lowlimit"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:Pattern"); }


                            //Duplicatable trueの場合はココで調整入る
                ...
                case "SkillPattern":
                    {
                        ...
                        foreach (var row in fcd.DataPart.DataWithoutColumnTitle)
                        {
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:skill1"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:skill2"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:skill3"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:skill4"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:skill5"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:weight1"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:weight2"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:weight3"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:weight4"); }
                            ...
                            if (!ret) { Debug.LogWarning($"Reflect Failed row:{i} col:weight5"); }


                            ///
                            MasterData.Instance.EnemyData.SkillPatternData[rowData.id] = rowData;
                        }
                    }
                    break;
